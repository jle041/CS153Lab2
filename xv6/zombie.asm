...
main(void)
{
  ...
  19:	e8 cc 02 00 00       	call   2ea <sleep>
  exit(1);
 ...
 145:	31 f6                	xor    %esi,%esi
{
 ...
 163:	e8 0a 01 00 00       	call   272 <read>
    if(cc < 1)
...
stat(const char *n, struct stat *st)
{
 ...
 1b6:	e8 df 00 00 00       	call   29a <open>
  if(fd < 0)
 ...
 1cb:	e8 e2 00 00 00       	call   2b2 <fstat>
  close(fd);
 1d0:	89 1c 24             	mov    %ebx,(%esp)
  r = fstat(fd, st);
 1d3:	89 c6                	mov    %eax,%esi
  close(fd);
 1d5:	e8 a8 00 00 00       	call   282 <close>
  return r;
...
000002fa <setpriority>:
SYSCALL(setpriority)
 2fa:	b8 17 00 00 00       	mov    $0x17,%eax
 2ff:	cd 40                	int    $0x40
 301:	c3                   	ret    

00000302 <getpriority>:
SYSCALL(getpriority)
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 38a:	e8 eb fe ff ff       	call   27a <write>
  while(--i >= 0)
 ...
 3be:	8d 45 10             	lea    0x10(%ebp),%eax
{
 ...
 3f3:	88 55 e2             	mov    %dl,-0x1e(%ebp)
  write(fd, &c, 1);
 3f6:	e8 7f fe ff ff       	call   27a <write>
 ...
 466:	e8 0f fe ff ff       	call   27a <write>
        putc(fd, c);
 ...
 480:	88 55 e7             	mov    %dl,-0x19(%ebp)
  write(fd, &c, 1);
 483:	e8 f2 fd ff ff       	call   27a <write>
  for(i = 0; fmt[i]; i++){
 ...
 510:	e8 65 fd ff ff       	call   27a <write>
        while(*s != 0){
 ...
 540:	e8 35 fd ff ff       	call   27a <write>
 ...
 590:	e8 e5 fc ff ff       	call   27a <write>
        ap++;
...
malloc(uint nbytes)
{
 ...
 69a:	e8 43 fc ff ff       	call   2e2 <sbrk>
  if(p == (char*)-1)
