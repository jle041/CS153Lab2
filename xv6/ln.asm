 ...
 195:	31 f6                	xor    %esi,%esi
{
 ...
 1b3:	e8 0a 01 00 00       	call   2c2 <read>
    if(cc < 1)
...
stat(const char *n, struct stat *st)
{
 ...
 206:	e8 df 00 00 00       	call   2ea <open>
  if(fd < 0)
 ...
 21b:	e8 e2 00 00 00       	call   302 <fstat>
  close(fd);
 220:	89 1c 24             	mov    %ebx,(%esp)
  r = fstat(fd, st);
 223:	89 c6                	mov    %eax,%esi
  close(fd);
 225:	e8 a8 00 00 00       	call   2d2 <close>
  return r;
...
0000034a <setpriority>:
SYSCALL(setpriority)
 34a:	b8 17 00 00 00       	mov    $0x17,%eax
 34f:	cd 40                	int    $0x40
 351:	c3                   	ret    

00000352 <getpriority>:
SYSCALL(getpriority)
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 3da:	e8 eb fe ff ff       	call   2ca <write>
  while(--i >= 0)
 ...
 40e:	8d 45 10             	lea    0x10(%ebp),%eax
{
 ...
 443:	88 55 e2             	mov    %dl,-0x1e(%ebp)
  write(fd, &c, 1);
 446:	e8 7f fe ff ff       	call   2ca <write>
 ...
 4b6:	e8 0f fe ff ff       	call   2ca <write>
        putc(fd, c);
 ...
 4d0:	88 55 e7             	mov    %dl,-0x19(%ebp)
  write(fd, &c, 1);
 4d3:	e8 f2 fd ff ff       	call   2ca <write>
  for(i = 0; fmt[i]; i++){
 ...
 560:	e8 65 fd ff ff       	call   2ca <write>
        while(*s != 0){
 ...
 590:	e8 35 fd ff ff       	call   2ca <write>
 ...
 5e0:	e8 e5 fc ff ff       	call   2ca <write>
        ap++;
